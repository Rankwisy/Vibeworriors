...
namespace VibeWarrior\Abilities\Gutenberg;
...
const POST_TYPE = 'vibewarrior_gb_change';

const KIND_BATCH = 'batch';

const KIND_ITEM = 'item';

const META_KIND = '_vibewarrior_gb_kind';

const META_STATUS = '_vibewarrior_gb_status';

const META_STATUS_UPDATED_AT = '_vibewarrior_gb_status_updated_at';

const META_AGENT_LABEL = '_vibewarrior_gb_agent_label';

const META_AGENT_SESSION_ID = '_vibewarrior_gb_agent_session_id';

const META_READY_AT = '_vibewarrior_gb_ready_at';

const META_FINALIZED_AT = '_vibewarrior_gb_finalized_at';

const META_LEASE_OWNER = '_vibewarrior_gb_lease_owner';

const META_LEASE_EXPIRES_AT = '_vibewarrior_gb_lease_expires_at';

const META_LAST_ERROR = '_vibewarrior_gb_last_error';

const META_TARGET_ID = '_vibewarrior_gb_target_id';

const META_TARGET_TYPE = '_vibewarrior_gb_target_type';

const META_OPERATION = '_vibewarrior_gb_operation';

const META_BASE_CONTENT_HASH = '_vibewarrior_gb_base_content_hash';

const META_BASE_CONTENT = '_vibewarrior_gb_base_content';

const META_BASE_REVISION_ID = '_vibewarrior_gb_base_revision_id';

const META_SPEC_HASH = '_vibewarrior_gb_spec_hash';

const META_BLOCK_SPEC = '_vibewarrior_gb_block_spec';

const META_VALIDATION_ERRORS = '_vibewarrior_gb_validation_errors';

const META_FINALIZATION_MODE = '_vibewarrior_gb_finalization_mode';

const META_FINALIZED_CONTENT = '_vibewarrior_gb_finalized_content';
...
const FINALIZER_RUNTIME_TRANSIENT = 'vibewarrior_gb_finalizer_runtimes';

const FINALIZER_RUNTIME_POLL_TOKEN_OPTION = 'vibewarrior_gb_finalizer_poll_token';
...
add_action('vibewarrior_gutenberg_cleanup', __NAMESPACE__ . '\\cleanup_queue');

function register_storage(): void
{
    register_post_type(POST_TYPE, [
        'label' => __('Gutenberg pending changes', domain: 'vibewarrior'),
        'public' => false,
        'show_ui' => false,
        'show_in_rest' => false,
...
{
    if (wp_next_scheduled('vibewarrior_gutenberg_cleanup') !== false) {
        return;
    }
    ...
    wp_schedule_event(
        ...
        recurrence: 'daily',
        hook: 'vibewarrior_gutenberg_cleanup',
    );
}

function unschedule_cleanup(): void
{
    wp_clear_scheduled_hook('vibewarrior_gutenberg_cleanup');
}
...
{
    $option = sprintf('vibewarrior_gb_status_lock_%d', $post_id);
...
{
    $label = trim($label) !== '' ? sanitize_text_field($label) : __('Untitled Gutenberg batch', domain: 'vibewarrior');
...
function target_title(WP_Post $target): string
{
    ...
    return $title !== '' ? $title : sprintf(__('(no title) #%d', domain: 'vibewarrior'), $target->ID);
}
...
function validate_dynamic_only_blocks(array $blocks): ?WP_Error
{
    foreach ($blocks as $block) {
        ...
        if (!is_vibewarrior_dynamic_block($name)) {
            return new WP_Error(
                'gutenberg_static_blocks_require_finalization',
                sprintf(
                    'Block "%s" is not a registered VibeWarrior-owned dynamic-only block. Native/static Gutenberg blocks must be queued with vibewarrior/gutenberg-add-pending-change and finalized in a browser before they are live.',
                    $name !== '' ? $name : '(missing name)',
                ),
                [
                    'status' => 400,
                    'finalization_required' => true,
                    'queue_ability' => 'vibewarrior/gutenberg-add-pending-change',
                    'enable_ability' => 'vibewarrior/gutenberg-enable-batch-finalization',
                ],
            );
        }
...
function is_vibewarrior_dynamic_block(string $name): bool
{
    if (!str_starts_with($name, 'vibewarrior/')) {
        return false;
    }
...
function item_change_summary(WP_Post $target, string $operation, array $blocks): string
{
    ...
    $name_summary = $names === []
        ? __('no blocks', domain: 'vibewarrior')
...
{
    return add_query_arg(['page' => 'vibewarrior-gutenberg-finalize'], admin_url('admin.php'));
}
...
function batch_label(WP_Post $batch): string
{
    ...
    return $label !== '' ? $label : sprintf(__('Gutenberg batch #%d', domain: 'vibewarrior'), $batch->ID);
}
...
function user_instruction(WP_Post $batch): string
{
    ...
    if (($runtime['online'] ?? false) === true && ($runtime['can_finalize_batch'] ?? false) === true) {
        return sprintf(
            'The VibeWarrior Block Editor Queue page is open and should automatically finalize Gutenberg batch #%d: %s. Do not ask the user to do anything unless the page goes offline; stream %s with curl -N, or poll %s with curl, until the batch becomes finalized, failed, or conflicted. Do not treat these Gutenberg changes as live until finalization completes.',
            $batch->ID,
            batch_label($batch),
    ...
    if (($runtime['online'] ?? false) === true) {
        return sprintf(
            'A VibeWarrior Block Editor Queue page is open, but that browser user may not be able to finalize Gutenberg batch #%d: %s. Ask the user to open %s as a user who can edit every target. Do not treat these Gutenberg changes as live until finalization completes.',
            $batch->ID,
            batch_label($batch),
    ...
    return sprintf(
        'The VibeWarrior Block Editor Queue page is not currently online. Ask the user to open %s and keep it open. It will automatically finalize Gutenberg batch #%d: %s when it can. Do not treat these Gutenberg changes as live until finalization completes.',
        finalizer_dashboard_url(),
        $batch->ID,
        batch_label($batch),
    );
...
function shape_item(WP_Post $item): array
{
    ...
    return [
        ...
        'target_title' => $target instanceof WP_Post
            ? target_title($target)
            : sprintf(__('Missing target #%d', domain: 'vibewarrior'), $target_id),
...
function conflict_payload(WP_Post $item): array
{
    ...
    return [
        ...
        'cancel_ability' => 'vibewarrior/gutenberg-delete-pending-batch',
...
{
    if (vibewarrior_current_user_can_manage()) {
        return true;
    }
...
function claim_batch(int $batch_id): array|WP_Error
{
    ...
    if ($current_status === STATUS_DRAFT) {
        return new WP_Error(
            ...
            'Draft Gutenberg batches cannot be finalized until vibewarrior/gutenberg-enable-batch-finalization is called.',
            ['status' => 409],
        );
    }
