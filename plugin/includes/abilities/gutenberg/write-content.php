...
wp_register_ability('vibewarrior/gutenberg-write-content', [
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'block_spec' => [
                'type' => 'array',
                'description' => 'Top-level Gutenberg block specs: [{name, attributes, innerBlocks}]. Only registered vibewarrior/* dynamic-only blocks are accepted here.',
    ...
    'meta' => [
        ...
        'mcp' => ['public' => true],
        'annotations' => [
            'instructions' => 'Use this only for VibeWarrior-owned dynamic-only Gutenberg blocks where save:null means no static saved HTML is needed. For static/native blocks, this ability refuses the write and tells you to use the pending queue and Block Editor Queue browser runtime.',
            'readonly' => false,
            'destructive' => true,
            'idempotent' => true,
        ],
...
function gutenberg_write_content(array $input): array|WP_Error
{
    ...
    return [
        ...
        'warnings' => [
            'Direct write path used because every block is a registered VibeWarrior-owned dynamic-only block.',
        ],
    ];
}
