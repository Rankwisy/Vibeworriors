...
{
    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/batches', args: [
        'methods' => 'GET',
        'callback' => __NAMESPACE__ . '\\rest_list_batches',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_access_dashboard',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/finalizer-runtime/heartbeat', args: [
        'methods' => 'POST',
        'callback' => __NAMESPACE__ . '\\rest_finalizer_runtime_heartbeat',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_access_dashboard',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/finalizer-runtime/status', args: [
        'methods' => 'GET',
    ...
    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/finalizer-runtime/events', args: [
        'methods' => 'GET',
        'callback' => __NAMESPACE__ . '\\rest_stream_finalizer_runtime_events',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_poll_finalizer_runtime',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/batches/(?P<batch_id>\d+)/claim', args: [
        'methods' => 'POST',
    ...
    register_rest_route(
        route_namespace: 'vibewarrior/v1',
    ...
    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/items/(?P<item_id>\d+)/spec', args: [
        'methods' => 'GET',
        'callback' => __NAMESPACE__ . '\\rest_get_item_spec',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_access_item',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/items/(?P<item_id>\d+)/complete', args: [
        'methods' => 'POST',
        'callback' => __NAMESPACE__ . '\\rest_complete_item',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_access_item',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/items/(?P<item_id>\d+)/fail', args: [
        'methods' => 'POST',
        'callback' => __NAMESPACE__ . '\\rest_fail_item',
        'permission_callback' => __NAMESPACE__ . '\\rest_can_access_item',
    ]);

    register_rest_route(route_namespace: 'vibewarrior/v1', route: '/gutenberg/batches/(?P<batch_id>\d+)/cancel', args: [
        'methods' => 'POST',
...
function rest_can_access_item(WP_REST_Request $request): bool|WP_Error
{
    ...
    if (!vibewarrior_current_user_can_manage() && ($target_id <= 0 || !current_user_can('edit_post', $target_id))) {
...
function rest_item_editor_url(WP_Post $item): string|WP_Error
{
    ...
    return add_query_arg([
        'vibewarrior_gb_finalizer' => '1',
        'vibewarrior_gb_item' => $item->ID,
    ], $editor_url);
}
