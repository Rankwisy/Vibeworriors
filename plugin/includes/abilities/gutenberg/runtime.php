...
function finalizer_runtime_startup_instruction(): string
{
    ...
    if (($runtime['online'] ?? false) === true) {
        return sprintf(
            'The VibeWarrior Block Editor Queue page is open. Keep %s open while Gutenberg static/native changes are queued and finalized. You can stream %s with curl -N, or poll %s with curl, to check whether the page is still online. If a later status shows finalizer_runtime.online=false, ask the user to reopen it before treating queued changes as live.',
...
function finalizer_runtime_poll_url(?int $batch_id = null): string
{
    return add_query_arg(
        ...
        rest_url('vibewarrior/v1/gutenberg/finalizer-runtime/status'),
    );
}
...
function finalizer_runtime_sse_url(?int $batch_id = null): string
{
    return add_query_arg(
        finalizer_runtime_url_args($batch_id),
        rest_url('vibewarrior/v1/gutenberg/finalizer-runtime/events'),
    );
}
...
{
    if (vibewarrior_user_can_manage($user_id)) {
        return true;
    }
