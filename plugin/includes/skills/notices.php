...
namespace VibeWarrior\Skills\Notices;
...
const RELOAD_NOTICE_TRANSIENT = 'vibewarrior_skill_pending_reload_notice';

const RELOAD_NOTICE_TTL = 60;

/**
 * Mark that the skill index has changed and a reload-MCP-client notice
 * should be shown to the admin the next time they land on a VibeWarrior
 ...
 *    current admin screen belongs to VibeWarrior.
 */
function render(): void
{
    ...
    $inline_key = 'vibewarrior_skill_admin_notice_' . get_current_user_id();
    ...
    // and we're on the Skills admin page specifically — other VibeWarrior
    // screens don't need this prompt.
    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if (!$screen instanceof \WP_Screen) {
        return;
    }
    if ($screen->id !== 'vibewarrior_page_' . Admin\PAGE_SLUG) {
        return;
    }
