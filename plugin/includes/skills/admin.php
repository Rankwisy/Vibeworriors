...
namespace VibeWarrior\Skills\Admin;

use VibeWarrior\Skills\Abilities\SkillWrite;
use VibeWarrior\Skills\Cpt;
use VibeWarrior\Skills\Notices;
...
const PAGE_SLUG = 'vibewarrior-skills';

function capability(): string
{
    return \vibewarrior_manage_capability();
}

function current_user_can_manage(): bool
{
    return \vibewarrior_current_user_can_manage();
}

/**
 * Skills lives as a submenu under the VibeWarrior top-level menu registered
 * in `vibewarrior.php`. The parent slug `vibewarrior-connect` is the canonical
...
{
    add_submenu_page(
        parent_slug: 'vibewarrior-connect',
        page_title: __('Skills', domain: 'vibewarrior'),
        menu_title: __('Skills', domain: 'vibewarrior'),
 ...
 * Hub (slug `vibewarrior-abilities`) inside the VibeWarrior menu group. Runs at a
...
{
    global $submenu;
    if (!is_array($submenu ?? null) || !is_array($submenu['vibewarrior-connect'] ?? null)) {
        return;
    }

    /** @var array<int, array<int, string>> $entries */
    $entries = $submenu['vibewarrior-connect'];
    $skills_entry = null;
    ...
    foreach ($entries as $entry) {
        $reordered[] = $entry;
        if (!$inserted && ($entry[2] ?? null) === 'vibewarrior-abilities') {
    ...
    $submenu['vibewarrior-connect'] = $reordered;
}

function render_page(): void
{
    if (!current_user_can_manage()) {
        wp_die(__('You do not have permission to manage skills.', domain: 'vibewarrior'));
    }
...
{
    $actions = [
        'vibewarrior_skill_create',
        'vibewarrior_skill_update',
        'vibewarrior_skill_delete',
        'vibewarrior_skill_restore',
        'vibewarrior_skill_permanent_delete',
        'vibewarrior_skill_toggle_status',
        'vibewarrior_skill_toggle_activation',
        'vibewarrior_skill_upload',
        'vibewarrior_skill_download',
        'vibewarrior_skill_download_all',
    ];
    foreach ($actions as $action) {
        $handler = 'handle_' . substr($action, strlen('vibewarrior_skill_'));
...
function enqueue_assets(string $hook): void
{
    ...
    // the sanitised menu_title of the parent top-level menu ("VibeWarrior").
    if ($hook !== 'vibewarrior_page_' . PAGE_SLUG) {
        return;
    }
    wp_enqueue_style(
        'vibewarrior-skills-admin',
        (string) VIBEWARRIOR_PLUGIN_URL . 'includes/skills/assets/admin.css',
        [],
        VIBEWARRIOR_VERSION,
    );
    ...
    wp_add_inline_script('code-editor', sprintf(
        'jQuery(function($){ var el = document.getElementById("vibewarrior-skills-content"); if (el && window.wp && wp.codeEditor) { var inst = wp.codeEditor.initialize(el, %s); window.vibewarriorSkillsEditor = inst && inst.codemirror ? inst.codemirror : null; window.dispatchEvent(new CustomEvent("vibewarrior-skills-editor-ready")); } });',
...
{
    require_capability_and_nonce('vibewarrior_skill_create');
...
function handle_update(): void
{
    ...
    require_capability_and_nonce('vibewarrior_skill_update_' . $post_id);
    ...
    if ($new_title === '') {
        redirect_with_notice(
            'error',
            __('Title must contain at least one letter or digit (lowercase, dash-separated).', domain: 'vibewarrior'),
            $post_id,
        );
        return;
    }
    ...
    if (trim($input['description']) === '') {
        redirect_with_notice('error', __('Description is required.', domain: 'vibewarrior'), $post_id);
        return;
    }

    $content = Parser\unescape_content($input['content']);
    if (strlen($content) > Parser\MAX_BODY_BYTES) {
        redirect_with_notice('error', __('Body exceeds 1 MB.', domain: 'vibewarrior'), $post_id);
        return;
    }
    ...
    if ($new_title !== $post->post_name) {
        ...
        if ($external !== null) {
            redirect_with_notice(
                ...
                sprintf(
                    ...
                    __('Title "%1$s" is already used by source "%2$s". Choose a different title.', domain: 'vibewarrior'),
                    $new_title,
                    $external,
                ),
                $post_id,
        ...
        if ($clash !== null && (int) $clash->ID !== $post_id) {
            redirect_with_notice('error', __('A skill with this title already exists.', domain: 'vibewarrior'), $post_id);
            return;
        }
    }
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('vibewarrior_skill_delete_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('vibewarrior_skill_restore_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('vibewarrior_skill_permanent_delete_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('vibewarrior_skill_toggle_status_' . $post_id);
    $post = load_skill_post($post_id);
...
function handle_toggle_activation(): void
{
    ...
    require_capability_and_nonce('vibewarrior_skill_toggle_activation_' . $post_id);
    if (!in_array($field, ['enable_prompt', 'enable_agentic'], strict: true)) {
        wp_die(__('Invalid field.', domain: 'vibewarrior'));
    }
...
{
    require_capability_and_nonce('vibewarrior_skill_upload');

    $files = normalize_uploaded_files($_FILES['skill_file'] ?? null);
    if ($files === []) {
        redirect_with_notice('error', __('No files uploaded.', domain: 'vibewarrior'));
        return;
    }
    if (count($files) > MAX_UPLOAD_FILES) {
        redirect_with_notice('error', sprintf(
            ...
            __('Too many files. Upload up to %d skills at a time.', domain: 'vibewarrior'),
            MAX_UPLOAD_FILES,
        ));
        return;
    }
    ...
    if ($imported !== []) {
        ...
        if ($imported_ids !== []) {
            set_transient('vibewarrior_skill_just_imported_' . get_current_user_id(), $imported_ids, expiration: 30);
        }
    }
...
function process_one_upload(array $file, string $on_conflict): string|\WP_Error
{
    if ($file['error'] !== UPLOAD_ERR_OK) {
        return new \WP_Error('upload_error', __('upload failed', domain: 'vibewarrior'));
    }
    if (!str_ends_with(strtolower($file['name']), '.md')) {
        return new \WP_Error('not_md', __('not a .md file', domain: 'vibewarrior'));
    }
    if ($file['size'] > Parser\MAX_BODY_BYTES) {
        return new \WP_Error('too_large', __('too large (max 1 MB)', domain: 'vibewarrior'));
    }
    if ($file['tmp_name'] === '' || !is_readable($file['tmp_name'])) {
        return new \WP_Error('unreadable', __('could not be read', domain: 'vibewarrior'));
    }
...
function build_upload_notice(array $imported, array $failed): string
{
    if ($imported === [] && $failed !== []) {
        return sprintf(
            ...
            __('Upload failed: %s', domain: 'vibewarrior'),
            implode(', ', $failed),
        );
    }
    $head = sprintf(
        /* translators: %d: count */
        _n('%d skill imported.', plural: '%d skills imported.', number: count($imported), domain: 'vibewarrior'),
        count($imported),
    );
    if ($failed === []) {
        return $head;
    ...
    return sprintf(
        ...
        __('%1$s Failed: %2$s', domain: 'vibewarrior'),
        $head,
        implode(', ', $failed),
    );
}
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('vibewarrior_skill_download_' . $post_id);
    $post = load_skill_post($post_id);
...
{
    require_capability_and_nonce('vibewarrior_skill_download_all');
    ...
    if ($posts === []) {
        redirect_with_notice('error', __('No skills to download.', domain: 'vibewarrior'));
        return;
    }

    $tmp = wp_tempnam('vibewarrior-skills-' . time() . '.zip');
    if ($tmp === '') {
        redirect_with_notice('error', __('Could not create temporary ZIP file.', domain: 'vibewarrior'));
        return;
    }
    ...
    if ($zip->open($tmp, \ZipArchive::OVERWRITE) !== true) {
        wp_delete_file($tmp);
        redirect_with_notice('error', __('Could not open the ZIP file for writing.', domain: 'vibewarrior'));
        return;
    }
    ...
    wp_delete_file($tmp);
    if ($raw === false) {
        redirect_with_notice('error', __('Could not read the generated ZIP.', domain: 'vibewarrior'));
        return;
    }

    $filename = sprintf('vibewarrior-skills-%s.zip', wp_date('Y-m-d'));
    nocache_headers();
...
{
    if (!current_user_can_manage()) {
        wp_die(__('Not allowed.', domain: 'vibewarrior'), title: '', args: ['response' => 403]);
    }
...
{
    set_transient(
        'vibewarrior_skill_admin_notice_' . get_current_user_id(),
