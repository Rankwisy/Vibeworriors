...
(function () {
    ...
    var hub = document.querySelector('.vibewarrior-hub');
    ...
    hub.addEventListener('submit', function (event) {
        ...
        if (!(form instanceof HTMLFormElement) || !form.matches('.vibewarrior-hub-actions form')) {
            return;
        }

        var actionInput = form.querySelector('input[name="vibewarrior_ability_hub_action"]');
        if (!actionInput || actionInput.value !== 'toggle_disabled') {
            return;
        }

        var row = form.closest('.vibewarrior-hub-row');
        ...
        body.set('action', 'vibewarrior_toggle_ability');
    ...
    hub.addEventListener('click', function (event) {
        if (event.target instanceof HTMLElement && event.target.closest('.vibewarrior-hub-select-all')) {
    ...
    hub.addEventListener('change', function (event) {
        ...
        if (input.classList.contains('vibewarrior-hub-select-all-input')) {
            var section = input.closest('.vibewarrior-hub-section, .vibewarrior-hub-subsection');
            if (section) {
                section.querySelectorAll('.vibewarrior-hub-row input[type="checkbox"]').forEach(function (box) {
        ...
        if (input.closest('.vibewarrior-hub-row')) {
            syncSelectAllStates();
        }
    });
    ...
    function syncSelectAllStates() {
        hub.querySelectorAll('.vibewarrior-hub-select-all-input').forEach(function (selectAll) {
            var section = selectAll.closest('.vibewarrior-hub-section, .vibewarrior-hub-subsection');
            if (!section) {
                return;
            }
            var boxes = section.querySelectorAll('.vibewarrior-hub-row input[type="checkbox"]');
            var checked = 0;
    ...
    var bulkForm = document.getElementById('vibewarrior-abilities-bulk');
    if (bulkForm) {
        bulkForm.addEventListener('submit', function (event) {
            if (selectedBulkAction() !== 'disable') {
                return;
            }
            var count = hub.querySelectorAll('.vibewarrior-hub-row input[type="checkbox"]:checked').length;
            if (count === 0) {
                return;
            }
    ...
    function selectedBulkAction() {
        var top = document.getElementById('vibewarrior-bulk-action-selector-top');
        var bottom = document.getElementById('vibewarrior-bulk-action-selector-bottom');
    ...
    function applyToggle(row, button, data) {
        ...
        var subsection = row.closest('.vibewarrior-hub-subsection');
        if (subsection) {
            refreshSectionMeta(subsection);
        }
        var section = row.closest('.vibewarrior-hub-section');
        if (section) {
    ...
    // "All disabled" pill, mirroring vibewarrior_render_ability_header_meta() in PHP.
    function refreshSectionMeta(section) {
        var summary = section.querySelector(':scope > summary');
        if (!summary) {
            return;
        }
        var rows = section.querySelectorAll('.vibewarrior-hub-row');
        ...
        var pill = summary.querySelector('.vibewarrior-hub-alloff');
        if (enabled === 0 && total > 0) {
            if (!pill && heading) {
                pill = document.createElement('span');
                pill.className = 'pill status is-disabled vibewarrior-hub-alloff';
