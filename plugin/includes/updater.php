 ...
 *
 * Checks https://license.dynamic.ooo/api/vibewarrior/info for new versions
...
add_filter('site_transient_update_plugins', callback: 'vibewarrior_check_for_updates');
add_filter('plugins_api', callback: 'vibewarrior_plugins_api', priority: 10, accepted_args: 3);
 ...
 * @return mixed
 */
function vibewarrior_check_for_updates($transient)
{
    ...
    $remote = vibewarrior_fetch_update_info();
    $plugin_file = plugin_basename(dirname(__DIR__) . '/vibewarrior.php');

    if ($remote === null || !version_compare(VIBEWARRIOR_VERSION, $remote['version'], operator: '<')) {
        // No remote info or up-to-date. Report as no_update so WordPress.org cannot override.
        $transient->no_update[$plugin_file] = (object) [
            'slug' => 'vibewarrior',
            'plugin' => $plugin_file,
            'new_version' => VIBEWARRIOR_VERSION,
            'url' => '',
            'package' => '',
        ];
 ...
 */
function vibewarrior_plugins_api($result, $action, $args)
{
    if ($action !== 'plugin_information' || ($args->slug ?? '') !== 'vibewarrior') {
        return $result;
    }
 ...
 */
function vibewarrior_fetch_update_info()
{
    $cache_key = 'vibewarrior_update_info';
    ...
    $raw = vibewarrior_request_update_info();
    if ($raw === null) {
        set_transient($cache_key, value: 'error', expiration: HOUR_IN_SECONDS);
        return null;
    }

    $data = vibewarrior_normalize_update_response($raw);
 ...
 */
function vibewarrior_request_update_info()
{
    $url =
        'https://license.dynamic.ooo/api/vibewarrior/info?'
        . http_build_query([
            's' => wp_parse_url(home_url(), PHP_URL_HOST),
            'v' => VIBEWARRIOR_VERSION,
        ]);
 ...
 */
function vibewarrior_normalize_update_response($raw)
{
    ...
    return [
        'name' => (string) ($raw['name'] ?? 'VibeWarrior'),
